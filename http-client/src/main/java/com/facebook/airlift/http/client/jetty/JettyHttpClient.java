        ...
        implements com.facebook.airlift.http.client.HttpClient
{
    ...
    private HttpRequest buildJettyRequest(Request finalRequest, JettyRequestListener listener)
    {
        ...
        if (bodyGenerator != null) {
            if (bodyGenerator instanceof StaticBodyGenerator) {
                ...
                try {
                    jettyRequest.body(new PathRequestContent(path));
                }
                catch (IOException e) {
                    throw new RuntimeException("Failed to create jetty request with path content", e);
                }
            }
            else {
    ...
    {
        HttpDestination poolingHttpDestination = (HttpDestination) destination;
